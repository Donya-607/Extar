#ifndef INCLUDED_EASING_H_
#define INCLUDED_EASING_H_

// --- 自前の関数群を持ってきたもの。constexprは消している ---
#include <array>
...
namespace Donya
{
	namespace Easing
	{
		...
		namespace Impl
		{
			...
			static float AdjustTimeIn( float t )
			{
				return t;
			}
			static float AdjustTimeOut( float t )
			{
				return 1.0f - t;
			}
			static float AdjustTimeInOut( float t )
			{
				return ( t < 0.5f )
					? 2.0f * t
					: 2.0f - ( 2.0f * t );
			}
			static float AdjustTime( Type type, float t )
			{
				if ( type == Type::In )
				{
			...
			static float AdjustResultIn( float result )
			{
				return result;
			}
			static float AdjustResultOut( float result )
			{
				return 1.0f - result;
			}
			static float AdjustResultInOut( float rawTime, float result )
			{
				return ( rawTime < 0.5f )
					? result * 0.5f
					: 1.0f - ( result * 0.5f );
			}
			static float AdjustResult( Type type, float rawTime, float result )
			{
				if ( type == Type::In )
				{
		...
		static const char *KindName( int easingKind )
		{
		...
		static const char *TypeName( int easingType )
		{
		...
		/// Please set 0.0f ~ 1.0f to "currentTime".
		/// </summary>
		static float Ease( CKind kind, Type type, float currentTime )
		{
